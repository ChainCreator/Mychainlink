...
function appendLiveChat(name, text, isMe) {
  ...
  if (isMe) { div.style.alignSelf = 'flex-end'; div.style.background = 'rgba(230,201,106,0.2)'; div.style.color = '#E6C96A'; }
...
function renderThread(){if(!activeThread)return;if(window.currentConvId&&sb&&supabaseOnline){renderThreadFromSupabase(window.currentConvId,activeThread);return}renderThreadLocalMsgs(conversations[activeThread]||[])}window.renderThread=renderThread;window.renderThreadLocalMsgs=function(msgs){msgs=msgs||[];var h='';msgs.forEach(function(m){var isSent=m.sent;h+='<div style="align-self:'+(isSent?'flex-end':'flex-start')+';max-width:75%">';h+='<div style="padding:10px 14px;border-radius:'+(isSent?'16px 16px 4px 16px':'16px 16px 16px 4px')+';background:'+(isSent?'linear-gradient(135deg,#F5E6A3,#E6C96A)':'#2A2A2A')+';color:'+(isSent?'#1C1C1C':'#F0F0F0')+';font-size:14px">'+esc(m.text)+'</div>';h+='<div style="font-size:10px;color:#666;margin-top:2px;text-align:'+(isSent?'right':'left')+'">'+new Date(m.ts).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})+'</div>';h+='</div>'});$('msg-bubbles').innerHTML=h;$('msg-bubbles').scrollTop=$('msg-bubbles').scrollHeight};
